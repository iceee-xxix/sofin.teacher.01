                ...
                <form action="{{route('delivery.addressSave')}}" method="post">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-12">
                            ...
                            <textarea rows="4" class="form-control" name="detail" id="detail" required>{{ old('detail', $info->detail) }}</textarea>
                        </div>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-md-12">
                    ...
                    <input type="hidden" id="lat" name="lat" value="{{ old('lat', $info->lat) }}">
                    <input type="hidden" id="lng" name="lng" value="{{ old('long', $info->long) }}">
                    <input type="hidden" id="id" name="id" value="{{ old('id', $info->id) }}">
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    function loadMapWithLocation() {
        const latFromServer = <?= $info->lat ?? 'null' ?>;
        const lngFromServer = <?= $info->long ?? 'null' ?>;

        let initialLocation;

        if (latFromServer && lngFromServer) {
            // ถ้ามี lat long ในฐานข้อมูล
            initialLocation = {
                lat: parseFloat(latFromServer),
                lng: parseFloat(lngFromServer),
            };
            initMapFromLocation(initialLocation);
    ...
    function initMapFromLocation(location) {
        map = new google.maps.Map(document.getElementById("map"), {
            center: location,
            zoom: 15,
        });
